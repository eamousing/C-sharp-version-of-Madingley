...
namespace Madingley
{
    ...
    public class AutotrophProcessor
    {
            ...
            bool specificLocations, uint currentMonth, Boolean nsfPhyto)
        {
            ...
            {
                // Check if size fractionated NPP is used
                if (nsfPhyto)
                {
                    ...
                    switch (gridCellStockHandler[actingStock].StockName)
                    {
                        ...
                        case "picophytoplankton":
                            ...
                            // Get picophytoplankton NPP from the cell environment
                            NPP = cellEnvironment["picoNPP"][currentMonth];

                            // If picophytoplankton NPP is a mssing value then set to zero
                            if (NPP == cellEnvironment["Missing Value"][0]) NPP = 0.0;

                            break;
                        case "nanophytoplankton":

                            // Get nanophytoplankton NPP from the cell environment
                            NPP = cellEnvironment["nanoNPP"][currentMonth];

                            // If nanophytoplankton NPP is a mssing value then set to zero
                            if (NPP == cellEnvironment["Missing Value"][0]) NPP = 0.0;

                            break;
                        case "microphytoplankton":

                            // Get microphytoplankton NPP from the cell environment
                            NPP = cellEnvironment["microNPP"][currentMonth];

                            // If microphytoplankton NPP is a mssing value then set to zero
                            if (NPP == cellEnvironment["Missing Value"][0]) NPP = 0.0;
